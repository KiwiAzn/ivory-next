...
create policy "Allow individual insert access" on public.users for insert with check( auth.uid() = user_id );
...
create policy "Allow individual insert access" on public.dice_rolls for insert with check ( auth.uid() = user_id );
...
begin
  ...
  values (new.id, new.raw_user_meta_data::json->>'display_name');
